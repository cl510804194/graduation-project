<template>
  <div style="width: 968px;">
    <el-form label-position="left" inline class="demo-table-expand">
      <el-tag type="info" class="birdgeint-tag">病害内容</el-tag>
      <div class="disease-content" v-for="(item,index) in treeItemDiseaseArray" :key="index">
        ...
        <el-form-item label="所属桥梁编码">
          <span>{{item.bridgeCode}}</span>
        </el-form-item>
        <el-form-item label="病害类别">
          <span>{{item.diseaseTypeCode}}</span>
        </el-form-item>
        <el-form-item label="病害实时标度">
          <span>{{item.diseaseGrade}}</span>
        </el-form-item>
        <el-form-item label="病害状态">
          <span>{{item.status}}</span>
        </el-form-item>
        <el-form-item label="处置建议">
          <span>{{item.disposalAdvise}}</span>
        </el-form-item>
        ...
        <el-form-item label="病害位置描述">
          <span>{{item.diseaseLocation}}</span>
        </el-form-item>
        <el-form-item label="病害定量描述">
          <span>{{item.quantifyDescription}}</span>
        </el-form-item>
        <el-form-item label="病害定性描述">
          <span>{{item.qualitativeDescription}}</span>
        </el-form-item>
      </div>
    </el-form>
  </div>
  <!-- <el-form label-width="110px" :model="menuForm" ref="menuForm" :rules="rules" size="mini" inline>
    <div class="inspection-content">
      <el-form-item label="病害名称 " prop="diseaseName">
        <el-select v-model="menuForm.diseaseName" placeholder="请选择病害">
          <el-option
            v-for="item in treeItemDiseaseArray"
      ...
      <el-form-item label="病害实时标度" prop="diseaseGrade">
        <el-select v-model="menuForm.diseaseGrade" placeholder="请选择病害实时标度">
          <el-option
...
export default {
  ...
  watch: {
    ...
    diseaseArray() {
      this.treeItemDiseaseArray = [...this.diseaseArray];
      console.log(this.treeItemDiseaseArray);
    }
  },
  data() {
    return {
      treeItemDiseaseArray: [],
